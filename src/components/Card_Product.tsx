...
export const Card_Product = ({ item }: { item: Product_Type }) => {
	...
	return (
		<li className="w-[270px] rounded-xl p-4 fondo-card-producto border-b-2 border-l-2 border-r-2 border-[#c62323] flex flex-col items-center justify-start">
			<div className="relative w-full h-5/6 my-4 group">
				<img loading="lazy" width={260} height={260} src={item.image_url ?? '/assets/img/mingcute--chat-3-fill.png'} alt={item.title} title={item.title} className="w-[260px] h-[260px] group-hover:opacity-15 duration-300 transition-all ease-in-out rounded-t-lg" />
			...
			<h3 className="text-principal w-full px-4 py-2 text-start text-lg font-bold">{item.title}</h3>
			<div className="w-full flex flex-row items-center justify-between">
				<a title={`https://wa.me/5358505864?text=Hola%20estoy%20interesado%20en:${item.id + "\n" + item.title + "\n" + item.price + "\n En: " + cambio.cambio_selected.value }`} href={`https://wa.me/5358505864?text=Hola%20estoy%20interesado%20en:\n${item.id + "\n" + item.title + "\n" + item.price + "\n En: " + cambio.cambio_selected.value }`} onClick={() => handleContactar(item)} type="button" className="px-4 py-2 bg-green-600 hover:bg-green-400 transition-all duration-300 ease-in-out rounded-b-lg">
					Contactar
				</a>
